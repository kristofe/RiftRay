...
// @var title Subterranean Fly-Through
// @var author Shane
// @var url https://www.shadertoy.com/view/XlXXWj
...
vec3 getSceneColor( in vec3 camPos, in vec3 rd )
{
	...
	light_pos2 = camPos + vec3(0.0, 0.0, 6.0);// Put it a bit in front of the camera.
...
void mainImage( out vec4 fragColor, in vec2 fragCoord ){
	...
	vec3 camPos = vec3(0.0, 0.0, -0.1); // Camera position, doubling as the ray origin.
    ...
    // RiftRay hack, t undo another riftray hack...
	camPos.xy -= path(camPos.z);

    // rd - Ray direction.
    vec3 rd = normalize(forward + FOV*uv.x*right + FOV*uv.y*up);
    
    fragColor = vec4(getSceneColor(camPos, rd), 1.0);
}
#endif
