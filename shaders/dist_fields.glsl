...
///////////////////////////////////////////////////////////////////////////////
    ...
    vec2 testRayAgainstScene( in vec3 pos){
      vec2 res = vec2( 
                  dfCylinder( pos-vec3( 0.1,-0.5,1.0), vec2(0.2,0.4) ) 
                    +
                  0.08  * turbulenceHighLacuniarity(pos* 2.0 + vec3(0.2*time))
                ,
                4.0 
               );
      ...
      res = Union( 
                  ...
                  vec2( 
                          dfSphere(
                                    pos-vec3(-0.9,-0.45,1.5),
                                    0.3
                                  )
                          + 
                          0.05 * noise(pos* 8.0 + vec3(time))
                        ,
                        3.0 
                      ) 
                  );
      res = Union( res, vec2( dfPlane(pos- vec3(0.0, -1.0, 0.0)), 6.0 ));
      return res;
    }
...
vec3 getSceneColor( in vec3 ori, in vec3 dir )
{
  Light light;
  light.direction = normalize(vec3(-0.6,0.9, -0.5));
